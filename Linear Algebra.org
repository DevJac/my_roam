...
An A x B matrix multiplied by a C x D matrix results in a A x D matrix, and B and C must be equal.
...
<2023-09-01> I finished chapter 16 about SVD. SVD is quite beutiful, it seems everything I've learned so far in the book has been necessary to understand SVD. I haven't solved many problems by hand, which might be useful. There's only two chapters left in the book, but I don't know if I will finish them.
